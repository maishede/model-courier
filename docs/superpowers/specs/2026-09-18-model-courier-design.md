...
实现状态：MVP 已在 2026-09-18 完成首轮落地和自测；本文中的资源数值仍是
默认限制，不是吞吐量或并发承诺。
...
当前实现已通过 34 个自动化测试，覆盖合同校验、SQLite 竞争领取、租约
过期重试、设备/Worker 权限、文件大小限制、SDK 重试、子进程超时和完整
Mock 链路；另用 Uvicorn 启动实例并验证了 `/healthz` 返回 `{"status":"ok"}`。

与完整设计目标相比，MVP 有以下明确边界：设备上传接口在一次成功上传后
直接完成 `uploading → queued`，`submit` 端点保留用于状态确认；当前结果以
结构化 JSON 保存在任务元数据中，Provider 返回的结果 artifact 清单尚未由
Worker API 持久化为独立文件；Worker 每次推理使用一个可终止子进程，因此
Provider 模型可能在任务之间重新加载。真实模型权重、GPU 性能、音频时长
和反向代理吞吐量尚未在本环境测量，不能把默认上限当成性能承诺。

成功发布结果后，控制平面会删除输入文件和对应 artifact 元数据；清理器
也会按 artifact 根目录解析相对路径，避免过期文件长期占用 40 GB 磁盘。
